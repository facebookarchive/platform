...
void nsCSSScanner::AddToError(const char *error)
{
  if (mError.IsEmpty()) {
    mErrorLineNumber = mLineNumber;
    mErrorColNumber = mColNumber;
    mError.AssignASCII(error);
  } else {
    mError.AppendLiteral("  ");
    mError.AppendASCII(error);
  }
}
...
const char *nsCSSScanner::GetError() {
  snprintf(mErrorMsg, MAX_ERROR_MSG_LENGTH,
           "CSS Error (line %u char %u): %s\n",
           mErrorLineNumber, mErrorColNumber,
