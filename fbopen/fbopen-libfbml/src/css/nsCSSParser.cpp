 ...
 *   Facebook, Inc. (Modifications Copyright © 2006-2008 Facebook, Inc.)
 *
...
protected:
  ...
  PRBool ParseFilter(nsresult& aErrorCode);
                                   ...
                                   PRUint32                 aLineNumber)
{
  ...
  nsresult rv =
    InitScanner(aAttributeValue, aDocURL, aLineNumber, aBaseURL);
  if (! NS_SUCCEEDED(rv)) {
    return rv;
  }
...
#else
  // since we don't parse @rules, these are all nsICSSStyleRules
  mSheet->AppendStyleRule((nsICSSStyleRule*)aRule);
#endif
}
...
#else
      list = new FBMLSelectorList(mSheet->GetContainerSelector(),
                                  mSheet->GetIdentifierPrefix());
#endif
      if (nsnull == list) {
        aErrorCode = NS_ERROR_OUT_OF_MEMORY;
        return PR_FALSE;
...
static void AppendSingleQuotedString(nsAutoString &out, nsAutoString &in) {
  out += '\'';
  nsAutoString::const_iterator start, end;
  for (in.BeginReading(start), in.EndReading(end); start != end; ++start) {
    PRUnichar ch = *start;
    if (ch != '\'' && ch != '"') out += ch;
  }
  out += '\'';
...
PRBool CSSParserImpl::ParseFilter(nsresult& aErrorCode) {
  nsAutoString s;
  for (;;) {
    if (!GetToken(aErrorCode, PR_TRUE)) {
      break;
    ...
    if (eCSSToken_Function == mToken.mType &&
        mToken.mIdent.LowerCaseEqualsLiteral("alpha")) {
      // skip alpha
      s += mToken.mIdent;
    } else {
      // skip progid:
      if (eCSSToken_Ident != mToken.mType ||
          !mToken.mIdent.LowerCaseEqualsLiteral("progid")) {
        REPORT_UNEXPECTED_TOKEN(PEUnknownProperty);
        return PR_FALSE;
      }
      if (!ExpectSymbol(aErrorCode, ':', PR_TRUE)) {
        REPORT_UNEXPECTED_TOKEN(PEParseDeclarationNoColon);
      ...
      if (!GetToken(aErrorCode, PR_TRUE) || eCSSToken_Ident != mToken.mType ||
          !mToken.mIdent.LowerCaseEqualsLiteral("dximagetransform") ||
          !ExpectSymbol(aErrorCode, '.', PR_TRUE) ||
          !GetToken(aErrorCode, PR_TRUE) || eCSSToken_Ident != mToken.mType ||
          !mToken.mIdent.LowerCaseEqualsLiteral("microsoft") ||
          !ExpectSymbol(aErrorCode, '.', PR_TRUE) ||
          !GetToken(aErrorCode, PR_TRUE) || eCSSToken_Function != mToken.mType ||
          (!mToken.mIdent.LowerCaseEqualsLiteral("alphaimageloader") &&
           !mToken.mIdent.LowerCaseEqualsLiteral("alpha"))) {
        REPORT_UNEXPECTED_TOKEN(PEUnknownProperty);
        return PR_FALSE;
      }

      s.AppendASCII("progid:DXImageTransform.Microsoft.");
      s += mToken.mIdent;
    }

    // find parameters for them
    if (!ExpectSymbol(aErrorCode, '(', PR_TRUE)) {
      break;
    }
    s += '(';
    ...
    for (;;) {
      ...
      PRInt32 aVariantMask;
      if (mToken.mIdent.LowerCaseEqualsLiteral("src")) {
        aVariantMask = VARIANT_URL;
      } else if (mToken.mIdent.LowerCaseEqualsLiteral("sizingmethod")) {
        aVariantMask = VARIANT_STRING;
      } else if (mToken.mIdent.LowerCaseEqualsLiteral("opacity")) {
      ...
      case VARIANT_URL:
        ...
        mSheet->TranslateURL(mToken.mIdent);
        AppendSingleQuotedString(s, mToken.mIdent);
        break;
      case VARIANT_STRING:
        if (!GetToken(aErrorCode, PR_TRUE) ||
            eCSSToken_String != mToken.mType) {
      ...
      case VARIANT_NUMBER:
        if (!GetToken(aErrorCode, PR_TRUE) ||
            eCSSToken_Number != mToken.mType) {
          REPORT_UNEXPECTED_TOKEN(PEUnknownProperty);
          return PR_FALSE;
        }
        s.AppendInt(mToken.mInteger, 10);
        break;
      default:
        break;
      }
  ...
  nsCSSValue aValue;
  aValue.SetStringValue(s, eCSSUnit_String);
  AppendValue(eCSSProperty_filter, aValue);
  return PR_TRUE;
}
...
#else
    values[2].SetColorValue(NS_RGB(255,255,255));
#endif
  }
