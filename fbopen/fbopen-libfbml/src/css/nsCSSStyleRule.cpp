...
nsCSSSelector::ToString(nsAString& aString, nsICSSStyleSheet* aSheet,
                        const nsString& aIdPrefix, PRBool aAppend) const
{
  if (!aAppend)
   aString.Truncate();

  ToStringInternal(aString, aSheet, IsPseudoElement(mTag), 0, aIdPrefix);
}
  ...
  if (!mTag && !mIDList && !mClassList) {
    ...
    if (mIDList) {
      ...
      while (list != nsnull) {
        ...
        NS_IF_NEGATED_START(aIsNegated, aString)
        aString.Append(PRUnichar('#'));
        if (!aIdPrefix.IsEmpty()) {
          // Some developers have already put appNNN as workarounds
          if (temp.Length() < 6 ||
              (temp[0] != 'a' || temp[1] != 'p' || temp[2] != 'p' ||
               temp[3] < '0' || temp[3] > '9' ||
               temp[4] < '0' || temp[4] > '9' ||
               temp[5] < '0' || temp[5] > '9')) {
            aString.Append(aIdPrefix);
            aString.Append(PRUnichar('_'));
          }
        }
        aString.Append(temp);
        NS_IF_NEGATED_END(aIsNegated, aString)
...
nsCSSSelectorList::ToString(nsAString& aResult, nsICSSStyleSheet* aSheet)
{
  ...
  nsString aIdPrefix;
  for (;;) {
    p->mSelectors->ToString(aResult, aSheet, aIdPrefix, PR_TRUE);
    p = p->mNext;
    if (!p)
      break;
