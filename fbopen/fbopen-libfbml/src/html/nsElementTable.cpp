...
nsHTMLElement *gHTMLElements = s_gHTMLElements;
static bool gHTMLElementsAlloced = false;

//static
void nsHTMLElement::ExpandTable(int extra) {
  NS_ASSERTION(extra > 0, "bad extra");

  int count = sizeof(s_gHTMLElements)/sizeof(nsHTMLElement);
  int new_count = count + extra;
  if (gHTMLElementsAlloced) {
    free(gHTMLElements);
  }
  gHTMLElements = (nsHTMLElement*)malloc(sizeof(nsHTMLElement) * new_count);
  gHTMLElementsAlloced = true;
  memcpy(gHTMLElements, s_gHTMLElements, sizeof(nsHTMLElement) * count);
  for (int i = count; i < new_count; i++) {
    memcpy(gHTMLElements + i, gHTMLElements + eHTMLTag_userdefined,
           sizeof(nsHTMLElement));
    gHTMLElements[i].mTagID = (nsHTMLTag)i;
    gHTMLElements[i].mInclusionBits = kFlowEntity|kSelf;
  }
}
