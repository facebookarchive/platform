...
nsresult CNavDTD::DidHandleStartTag(nsIParserNode& aNode,eHTMLTags aChildTag){
  ...
  if(nsHTMLElement::IsContainer(aChildTag) && theNode && theNode->mToken) {  //nullptr test fixes bug 56085
    ...
    if(theToken->IsEmpty() || theToken->IsSelfClosed()){
...
#ifdef FBML
  // if FBML tags are involved, no omission is allowed
  if (aParent >= eHTMLTag_userdefined ||
      aChild >= eHTMLTag_userdefined) return PR_FALSE;
#endif
