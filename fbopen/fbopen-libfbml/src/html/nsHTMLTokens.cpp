                      ...
                      PRBool translateEntity)
{
  ...
  do {
    ...
    if(NS_SUCCEEDED(result)) {
      PRUnichar ch;
      aScanner.Peek(ch);
      if(ch == kAmpersand && translateEntity) {
                                                ...
                                                PRBool& aFound)
{
  ...
  do {
    result = ConsumeUntil(theContent, mNewlineCount, aScanner,
                          theEndCondition, PR_TRUE, aFlag, PR_FALSE);
                             ...
                             PRInt32 aFlag)
{
  ...
  result=ConsumeUntil(aString,aNewlineCount,aScanner,
                      *terminateCondition,PR_TRUE,aFlag,PR_TRUE);
  ...
  if(!aString.str().IsEmpty() && aString.str().Last()!=aChar &&
    ...
    result=ConsumeUntil(aString,aNewlineCount,aScanner,
                        theAttributeTerminator,PR_FALSE,aFlag,PR_TRUE);
    if (NS_SUCCEEDED(result) && (aFlag & NS_IPARSER_FLAG_VIEW_SOURCE)) {
        ...
        if (NS_OK==result) {
          if (kEqual==aChar){
            ...
            if (kQuote == aChar || kApostrophe == aChar ||
                kForwardSlash == aChar) {
#ifdef FBML
              if (aChar == kForwardSlash) {
                result = aScanner.SkipOver(aChar); // slash.
                if (NS_SUCCEEDED(result)) {
                  result = aScanner.SkipWhitespace(mNewlineCount);
                }
                return NS_ERROR_HTMLPARSER_FAKE_ENDTAG;
              }
#endif
