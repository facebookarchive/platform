...
#define DHOOK(s)                                                        \
    (cx->decompileHookFunc ?                                            \
     cx->decompileHookFunc(cx->decompileHookObject,op,s,0) : s)
#define PROPHOOK(s)                                                     \
    (cx->decompileHookFunc ?                                            \
     cx->decompileHookFunc(cx->decompileHookObject,-1,s,0) : s)
#define ELEMHOOK(s)                                                     \
    (cx->decompileHookFunc ?                                            \
     cx->decompileHookFunc(cx->decompileHookObject,-2,s,0) : s)
    ...
    *hole = JS_FALSE;
    ...
    switch (op) {
      ...
      case JSOP_SETLOCALPOP:
        ...
        LOCAL_ASSERT(lval);
        todo = SprintCString(&ss->sprinter, DHOOK(lval));
...
#define GET_ATOM_QUOTE_AND_FMT(qfmt, ufmt, rval)                              \
    ...
    while (nb < 0 || pc < endpc) {
        ...
        if (op == JSOP_ENTERWITH && cx->decompileHookFunc) {
            int lineno = JS_PCToLineNumber(cx, jp->script, pc);
            cx->decompileHookFunc(cx->decompileHookObject, op, "", lineno);
        }
        ...
        if (cs->token) {
            switch (cs->nuses) {
              ...
              case 0:
                todo = SprintCString(&ss->sprinter, DHOOK(cs->token));
                break;
...
#define END_LITOPX_CASE                                                       \
              ...
              do_forinhead:
                ...
                if (atom) {
                    ...
                    if (js_CodeSpec[lastop].format & JOF_XMLNAME) {
                        ok = (Sprint(&ss->sprinter, ".%s", PROPHOOK(xval))
                              >= 0);
                    } else {
                        ok = (Sprint(&ss->sprinter, "[%s]", ELEMHOOK(xval))
                              >= 0);
                    }
              ...
              case JSOP_DELPROP:
                ...
                lval = POP_STR();
                todo = Sprint(&ss->sprinter, fmt, js_delete_str, lval,
                              ATOM_IS_IDENTIFIER(atom) ?
                              PROPHOOK(rval) : ELEMHOOK(rval));
                break;
              ...
              case JSOP_DELELEM:
                ...
                if (js_CodeSpec[lastop].format & JOF_XMLNAME) {
                    todo = Sprint(&ss->sprinter, "%s %s.%s",
                                  js_delete_str, lval, PROPHOOK(xval));
                } else {
                    todo = Sprint(&ss->sprinter, "%s %s[%s]",
                                  js_delete_str, lval, ELEMHOOK(xval));
                }
                break;
              ...
              do_setprop_rval:
                ...
                lval = POP_STR();
                sn = js_GetSrcNote(jp->script, pc - 1);
                todo = Sprint(&ss->sprinter, fmt, lval,
                              ATOM_IS_IDENTIFIER(atom) ?
                              PROPHOOK(xval) : ELEMHOOK(xval),
