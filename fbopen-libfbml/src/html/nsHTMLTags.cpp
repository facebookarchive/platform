...
#define NS_HTMLTAG_NAME_MAX_LENGTH 32

static bool kExpandedTagUnicodeTableInited = false;
static const PRUnichar* kExpandedTagUnicodeTable[512];
static const PRUnichar* kExpandedTagUnicodeUnderScoreTable[512];
...
void nsHTMLTags::RemoveExpandedTags() {
  if (!kExpandedTagUnicodeTableInited) return;

  for (int index = 0; index <  sizeof(kExpandedTagUnicodeTable)/sizeof(kExpandedTagUnicodeTable[0]); index++) {
...
{
  if (aEnum > eHTMLTag_userdefined) {
    return kExpandedTagUnicodeTable[aEnum - eHTMLTag_userdefined];
  }
